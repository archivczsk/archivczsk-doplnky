...
NAME_PREFIX="magiogo"
NAME = "MagioGO"
ADDON_NAME='plugin.video.magiogo'
...
class MagioGoBouquetGenerator(BouquetGeneratorTemplate):
...
def create_xmlepg( magiogo, data_file, channels_file, days ):
	...
	with open( channels_file, "w" ) as fc:
		with open( data_file, "w" ) as f:
			...
			epg_ids = [int(channel.id) for channel in magiogo.get_channel_list()]
			
			for channel in magiogo.get_channel_list():
				id_content = NAME + '_' + strip_accents(channel.name).replace(' ', '_').replace('"','').replace(':','').replace('/','').replace('.','')
				fc.write( ' <channel id="%s">1:0:1:%X:%X:%X:%X:0:0:0:http%%3a//</channel>\n' % (id_content, SERVICEREF_SID_START + int(channel.id), SERVICEREF_TID, SERVICEREF_ONID, SERVICEREF_NAMESPACE))
				
			for day in range(days):
				fromts = int(time()) + (day * 86400)
				tots = (int(time()) + ((day+1) * 86400))
				
				epg_data = magiogo.get_channels_epg( epg_ids, fromts, tots )
				...
				for channel in magiogo.get_channel_list():
					...
					for epg_item in epg_data:
						if int(channel.id) == epg_item.get('channel',{}).get('channelId'):
					...
					for event in epg:
						try:
							xml_data = {
								'start': datetime.utcfromtimestamp( event['startTimeUTC'] / 1000 ).strftime('%Y%m%d%H%M%S') + ' 0000',
								'stop': datetime.utcfromtimestamp( event['endTimeUTC'] / 1000 ).strftime('%Y%m%d%H%M%S') + ' 0000',
								'title': escape(str(event['program']['title'])),
								'desc': escape(event['program']["description"]) if event['program'].get("description") != None else None
							}
...
def generate_xmlepg_if_needed(settings):
	...
	magiogo = init_magiogo( settings )
	if magiogo == None:
		service_helper.logInfo("No magiogo login credentials provided or they are wrong")
		return

	# time to generate new XML EPG file
	try:
		gen_time_start = time()
		create_xmlepg(magiogo, data_file, channels_file, int(settings['xmlepg_days']))
		data_mtime = time()
...
def start_epg_generator(arg):
	...
	service_helper.getSettings(['region', 'username', 'password', 'deviceid', 'devicetype', 'enable_userbouquet', 'enable_xmlepg', 'xmlepg_dir', 'xmlepg_days'], settings_received_epg )
...
def settings_received_epg( settings ):
	...
	if not settings['username'] or not settings['password'] or not settings['deviceid'] or not settings['xmlepg_dir']:
		epg_generator_running = False
...
def http_endpoint_received( addon_id, endpoint, force, stop_service ):
	...
	# load actual settings and continue when received
	service_helper.getSettings(['region', 'username', 'password', 'deviceid', 'devicetype', 'enable_userbouquet', 'enable_adult', 'enable_xmlepg', 'player_name', 'enable_picons'], settings_received_bouquet, endpoint=endpoint, force=force, stop_service=stop_service )
...
def settings_received_bouquet( settings, endpoint, force, stop_service ):
	...
	if not settings['username'] or not settings['password'] or not settings['deviceid']:
		service_helper.logError("No login data provided")
...
def remove_userbouquet(endpoint):
	obg = MagioGoBouquetGenerator( endpoint )
...
def generate_userbouquet( data ):
	...
	try:	
		...
		if force:
			msg = "Pri generovaní userbouquetu pre MagioGO nastala chyba. Skontrolujte log súbor a zareportujte chybu."
...
class MagioGoAddonServiceHelper( AddonServiceHelper ):
...
service_helper = StartAddonServiceHelper(MagioGoAddonServiceHelper(), start_epg_generator, None)
