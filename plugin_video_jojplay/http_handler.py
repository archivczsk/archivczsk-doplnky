...
class JojPlayHlsMasterProcessor(HlsMasterProcessor):
	def cleanup_master_playlist(self):
		super(JojPlayHlsMasterProcessor, self).cleanup_master_playlist()
...
class JojPlayHTTPRequestHandler(HlsHTTPRequestHandler, DashHTTPRequestHandler):
	def __init__(self, content_provider, addon ):
		...
		self.hls_master_processor = JojPlayHlsMasterProcessor
