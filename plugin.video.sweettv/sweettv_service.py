...
NAME_PREFIX="sweettv"
NAME = "SweetTV"
ADDON_NAME='plugin.video.sweettv'
...
class SweetTvBouquetGenerator(BouquetGeneratorTemplate):
...
def init_sweettv( settings ):
	...
	if len(settings['username']) > 0 and len( settings['password'] ) > 0:
		try:
			sweettv = SweetTvCache.get( settings['username'], settings['password'], settings['device_id'], profile_dir, service_helper.logInfo )
		except Exception as e:
			service_helper.logError("Failed to init Sweet.TV client: %s" % str(e))
			sweettv = None
	else:
		sweettv = None
...
def create_xmlepg( sweettv, data_file, channels_file, days ):
	...
	with open( channels_file, "w" ) as fc:
		with open( data_file, "w" ) as f:
			...
			channels = sweettv.get_channels_sorted()
			epgdata = sweettv.get_epg( limit_next=1000 )

			for channel in channels:
				id_content = NAME + '_' + channel['slug']
			...
			for channel in channels:
				service_helper.logInfo("Processing EPG for channel: %s (%s)" % (channel['name'], channel['id']))
				...
				for event in epgdata.get(channel['id'], []):
					try:
						xml_data = {
							'start': datetime.utcfromtimestamp( event['time_start'] ).strftime('%Y%m%d%H%M%S') + ' 0000',
							'stop': datetime.utcfromtimestamp( event['time_stop'] ).strftime('%Y%m%d%H%M%S') + ' 0000',
							'title': escape(str(event['text'])),
							'desc': ' '
						}
...
def generate_xmlepg_if_needed(settings):
	...
	sweettv = init_sweettv( settings )
	if sweettv == None:
		service_helper.logInfo("No SweetTV login credentials provided or they are wrong")
		return

	# time to generate new XML EPG file
	try:
		gen_time_start = time()
		create_xmlepg(sweettv, data_file, channels_file, int(settings['xmlepg_days']))
		data_mtime = time()
...
def start_epg_generator(arg):
	...
	service_helper.getSettings(['username', 'password', 'device_id', 'enable_userbouquet', 'enable_xmlepg', 'xmlepg_dir', 'xmlepg_days'], settings_received_epg )
...
def settings_received_epg( settings ):
	...
	if not settings['username'] or not settings['password'] or not settings['device_id'] or not settings['xmlepg_dir']:
		epg_generator_running = False
...
def http_endpoint_received( addon_id, endpoint, force, stop_service ):
	...
	# load actual settings and continue when received
	service_helper.getSettings(['username', 'password', 'device_id', 'enable_userbouquet', 'enable_adult', 'enable_xmlepg', 'player_name', 'enable_picons'], settings_received_bouquet, endpoint=endpoint, force=force, stop_service=stop_service )
...
def settings_received_bouquet( settings, endpoint, force, stop_service ):
	...
	if not settings['username'] or not settings['password'] or not settings['device_id']:
		service_helper.logError("No login data provided")
...
def remove_userbouquet(endpoint):
	obg = SweetTvBouquetGenerator( endpoint )
...
def generate_userbouquet( data ):
	...
	if sweettv and not sweettv.check_login():
		sweettv = None
	...
	try:	
		...
		for channel in sweettv.get_channels_sorted():
			channels.append({
					'id': int(channel['id']),
					'key': channel['id'],
					'name': channel['name'],
		...
		if force:
			msg = "Pri generovaní userbouquetu pre SweetTV nastala chyba. Skontrolujte log súbor a zareportujte chybu."
...
class SweetTvAddonServiceHelper( AddonServiceHelper ):
...
service_helper = StartAddonServiceHelper(SweetTvAddonServiceHelper(), start_epg_generator, None)
