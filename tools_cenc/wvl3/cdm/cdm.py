...
from tools_cenc.google.protobuf.message import DecodeError
from tools_cenc.google.protobuf import text_format
...
class Cdm:
	CERTIFICATE_CHALLENGE = b"\x08\x04"
	...
	def _parse_init_data(self, init_data_b64):
		parsed_init_data = wv_proto4_pb2.WidevineCencHeader()
		try:
	...
	def set_service_certificate(self, session_id, cert_raw):
		if cert_raw == None:
			return 1

#		self.logger.debug("set_service_certificate(session_id={}, cert={})".format(session_id, base64.b64decode(cert_raw).decode('urf-8')))
		...
		message = wv_proto4_pb2.SignedMessage()

		try:
			message.ParseFromString(cert_raw)
		except DecodeError:
			self.logger.error("failed to parse cert as SignedMessage")

		service_certificate = wv_proto4_pb2.SignedDeviceCertificate()
		...
		if message.Type:
			...
			try:
				service_certificate.ParseFromString(cert_raw)
			except DecodeError:
				self.logger.error("failed to parse service certificate")
				return 1
	...
	def get_license_request(self, session_id):
		...
		if self.raw_pssh:
			license_request = wv_proto4_pb2.SignedLicenseRequestRaw()
		else:
			license_request = wv_proto4_pb2.SignedLicenseRequest()
		client_id = wv_proto4_pb2.ClientIdentification()

		if not session.device_config.device_client_id_blob:
			self.logger.error("no client ID blob available for this device")
			return 1

		try:
			cid_bytes = client_id.ParseFromString(session.device_config.device_client_id_blob)
		except DecodeError:
		...
		if not self.raw_pssh:
			license_request.Type = wv_proto4_pb2.SignedLicenseRequest.MessageType.Value('LICENSE_REQUEST')
			license_request.Msg.ContentId.CencId.Pssh.CopyFrom(session.init_data)
		else:
			license_request.Type = wv_proto4_pb2.SignedLicenseRequestRaw.MessageType.Value('LICENSE_REQUEST')
			license_request.Msg.ContentId.CencId.Pssh = session.init_data # bytes

		if session.offline:
			license_type = wv_proto4_pb2.LicenseType.Value('OFFLINE')
		else:
			license_type = wv_proto4_pb2.LicenseType.Value('DEFAULT')
		license_request.Msg.ContentId.CencId.LicenseType = license_type
		license_request.Msg.ContentId.CencId.RequestId = session_id
		license_request.Msg.Type = wv_proto4_pb2.LicenseRequest.RequestType.Value('NEW')
		license_request.Msg.RequestTime = int(time.time())
		license_request.Msg.ProtocolVersion = wv_proto4_pb2.ProtocolVersion.Value('CURRENT')
		...
		if session.privacy_mode:
			if session.device_config.vmp:
				...
				vmp_hashes = wv_proto4_pb2.FileHashes()
				try:
					vmp_bytes = vmp_hashes.ParseFromString(session.device_config.device_vmp_blob)
				except DecodeError:
			...
			encrypted_client_id_proto = wv_proto4_pb2.EncryptedClientIdentification()
		...
		if session.device_config.private_key_available:
			key = RSA.importKey(session.device_config.device_private_key)
	...
	def provide_license(self, session_id, license_raw):
#		self.logger.debug("provide_license(session_id={}, license_b64={})".format(session_id, base64.b64encode(license_raw).decode('utf-8')))
		...
		license = wv_proto4_pb2.SignedLicense()
		try:
			license.ParseFromString(license_raw)
		except DecodeError:
...
#				f.write(license.SerializeToString())
		...
		for key in license.Msg.Key:
			if key.Id:
				...
				key_id = wv_proto4_pb2.License.KeyContainer.KeyType.Name(key.Type).encode('utf-8')
			...
			if type == "OPERATOR_SESSION":
				...
				self.logger.debug("Permissions: {}".format(str(permissions)))
			else:
				permissions = []
