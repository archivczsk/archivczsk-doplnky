...
NAME_PREFIX="rebittv"
NAME = "RebitTV"
ADDON_NAME='plugin.video.rebittv'
...
class RebitTvBouquetGenerator(BouquetGeneratorTemplate):
...
def init_rebittv( settings ):
	...
	if len(settings['username']) > 0 and len( settings['password'] ) > 0:
		try:
			rebittv = RebitTvCache.get( settings['username'], settings['password'], settings['device_name'], profile_dir, service_helper.logInfo )
		except Exception as e:
			service_helper.logError("Failed to init Rebit.TV client: %s" % str(e))
			rebittv = None
	else:
		rebittv = None
...
def create_xmlepg( rebittv, data_file, channels_file, days ):
	fromts = int(time())-7200
	tots = (int(time()) + (days * 86400))
	
	epg_current = rebittv.get_current_epg()
	...
	with open( channels_file, "w" ) as fc:
		with open( data_file, "w" ) as f:
			...
			channels = rebittv.get_channels_sorted()
			...
			for channel in channels:
				...
				fc.write( ' <channel id="%s">1:0:1:%X:%X:%X:%X:0:0:0:http%%3a//</channel>\n' % (id_content, SERVICEREF_SID_START + int(channel['number']), SERVICEREF_TID, SERVICEREF_ONID, SERVICEREF_NAMESPACE))
				
				service_helper.logInfo("Processing EPG for channel: %s" % channel['name'])
				
				fromts2 = epg_current.get(channel['id'],{}).get('start', fromts) - 1 
				epgdata = rebittv.get_epg( channel['id'], fromts2, tots )
				
				for event in epgdata:
					try:
						xml_data = {
							'start': datetime.utcfromtimestamp( event['start'] ).strftime('%Y%m%d%H%M%S') + ' 0000',
							'stop': datetime.utcfromtimestamp( event['stop'] ).strftime('%Y%m%d%H%M%S') + ' 0000',
							'title': escape(str(event['title'])),
							'desc': escape(str(event['description'])) + ' '
						}
...
def generate_xmlepg_if_needed(settings):
	...
	rebittv = init_rebittv( settings )
	if rebittv == None:
		service_helper.logInfo("No RebitTV login credentials provided or they are wrong")
		return

	# time to generate new XML EPG file
	try:
		gen_time_start = time()
		create_xmlepg(rebittv, data_file, channels_file, int(settings['xmlepg_days']))
		data_mtime = time()
...
def start_epg_generator(arg):
	...
	service_helper.getSettings(['username', 'password', 'device_name', 'enable_userbouquet', 'enable_xmlepg', 'xmlepg_dir', 'xmlepg_days'], settings_received_epg )
...
def settings_received_epg( settings ):
	...
	if not settings['username'] or not settings['password'] or not settings['device_name']:
		epg_generator_running = False
...
def http_endpoint_received( addon_id, endpoint, force, stop_service ):
	...
	# load actual settings and continue when received
	service_helper.getSettings(['username', 'password', 'device_name', 'enable_userbouquet', 'enable_adult', 'enable_xmlepg', 'player_name', 'enable_picons'], settings_received_bouquet, endpoint=endpoint, force=force, stop_service=stop_service )
...
def remove_userbouquet(endpoint):
	obg = RebitTvBouquetGenerator( endpoint )
...
def generate_userbouquet( data ):
	...
	if rebittv and not rebittv.check_login():
		rebittv = None
	...
	try:	
		...
		for channel in rebittv.get_channels_sorted():
			channels.append({
		...
		if force:
			msg = "Pri generovaní userbouquetu pre RebitTV nastala chyba. Skontrolujte log súbor a zareportujte chybu."
...
class RebitTvAddonServiceHelper( AddonServiceHelper ):
...
service_helper = StartAddonServiceHelper(RebitTvAddonServiceHelper(), start_epg_generator, None)
