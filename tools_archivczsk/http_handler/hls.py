...
class HlsMasterProcessor(HlsPlaylist):
	def __init__(self, request_handler, url, hls_info=None):
		...
		self.enable_multiaudio = (not hasattr(request_handler, 'enable_hls_multiaudio')) or request_handler.enable_hls_multiaudio
	...
	def cleanup_master_playlist(self):
		super(HlsMasterProcessor, self).cleanup_master_playlist()

		if not self.enable_multiaudio or not self.hls_info.get('enable_multiaudio', True):
